...
--- << typographic >> ... symbols --
    ...
    expect(parsed)
      .to eq("#{symbols[:mdash]} #{symbols[:laquo]} typographic " \
             "#{symbols[:raquo]} #{symbols[:hellip]} symbols " \
             "#{symbols[:ndash]}\n")
  end
  ...
  it "converts smart quotes to utf-8 chars" do
    ...
    expect(parsed)
      .to eq("To #{symbols[:ldquo]}extract#{symbols[:rdquo]} " \
             "\e[33mscript.rb\e[0m#{symbols[:rsquo]}s#{symbols[:hellip]}\n")
  end
end
